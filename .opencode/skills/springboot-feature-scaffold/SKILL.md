...
- Run `./gradlew build` in each project to verify compilation
...
| Tests | `*Test.java`, JUnit 5, Mockito | `` *Test.kt ``, JUnit 5, MockK | `*Spec.groovy`, Spock |
...
10. **Test files:** Mirror the structure from `film` tests.
    - **Controller test**: `@WebMvcTest({Domain}RestController)`, `@Import(ControllerAdvice.class, {Domain}RestMapperImpl.class)` (Java/Kotlin) or `@Import([ControllerAdvice, {Domain}RestMapper, ModelMapperConfig])` (Groovy). Mock the in-port. Test implemented operations for `200 OK` and unimplemented ones for `500 INTERNAL_SERVER_ERROR`.
    - **Service test**: Mockito/MockK/Spock. Mock the out-port. Verify delegation.
    - **Repository test**: `@DataJpaTest` + `@Import({Domain}RepositoryImpl.class, {Domain}JpaMapperImpl.class)` (Java/Kotlin) or `@Import({Domain}RepositoryImpl, {Domain}JpaMapper)` (Groovy). Use `@ActiveProfiles("test")`.

**For NEW domains, if JPA entity is NOT available:**

Generate files 1, 2, 5, 6, and 10 (controller test only). **DO NOT generate** files 3, 4, 7, 8, 9, or the repository/service tests.
...
3. Update/add **tests** for the newly implemented operations.
...
For each of the three projects, run `./gradlew build` from the project root (e.g., `spring_java/`, `spring_kotlin/`, `spring_groovy/`).
...
6. **Next steps**:
   ...
   - If build failed: show the first few errors and suggest fixes.
